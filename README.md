...
- Terminal.app, [iTerm2][iterm2], or Ghostty on macOS, and any terminal
  ...
  detecting whether their specific tab is active; Ghostty on macOS can only
  detect whether the whole app is frontmost. Linux (including Ghostty on
  Linux) always notifies — no frontmost detection is attempted there.

- [noti][noti] is preferred for posting to macOS Notification Center or, on
  Linux, to the desktop's freedesktop notification service.
...
Also, the plugin looks up `noti` on your `$PATH`, falling back to
`/usr/local/bin/noti` if it isn't found there, and assumes `growlnotify` is
...
environment variable to "true" to change focus to the terminal emulator
window when a notification is posted. By default the terminal window will
just demand attention.


[growlnotify]: http://growl.info/extras.php/#growlnotify
[noti]: https://codeberg.org/roble/noti
